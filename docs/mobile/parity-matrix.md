...
| Fault-injection control (presenter demo tool) | ✅ | ❌ | ❌ | **Deliberate, not a gap to close** — see [`fault-injection-decision.md`](./fault-injection-decision.md). Web is served fresh per demo session and never installed on a device; a mobile binary is an artifact a customer could end up holding, and shipping a "break this backend provider" switch inside it is a bigger liability than the convenience it buys a presenter. Both mobile clients still correctly *render* whatever fault state the backend is actually in — they're outcome-complete, just not trigger-complete. |
