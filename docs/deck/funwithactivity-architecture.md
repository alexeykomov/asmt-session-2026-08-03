...
- All three render `ok`/`skipped`/`degraded` provider states with distinct styling, verified on a live six-row response from both real vendors. **Correction as of the three-tab rebuild:** fault injection now lives in every client's Profile → DEVELOPER section, not web-only — the toggles that used to sit only on the web form's fieldset are now part of the shared Profile screen on iOS and Android too. The liability argument this bullet used to make (a mobile binary a customer could hold, carrying a "break this backend provider" switch) is worth re-raising with the team rather than restated here as still true.
