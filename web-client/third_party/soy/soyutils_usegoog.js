 ...
 *
 * Source: node_modules/closure-templates@20160825.0.0/soyutils_usegoog.js
 * (see web-client/scripts/compile.js, which points here instead of at
 * node_modules so `npm install` never silently reverts these patches).
 * This runtime predates google-closure-library's removal of
 * `goog.soy.data.UnsanitizedText` and `SanitizedContentKind.TEXT` (both
 * gone by the pinned ^20230802.0.0), and its deprecated `soy.renderElement`
 * / `soy.renderAsFragment` / `soy.renderAsElement` aliases no longer match
 * modern `goog.soy`'s type signatures. Rather than downgrade
 * google-closure-library (which then fails to satisfy the pinned, far
 * newer google-closure-compiler's own goog.define expectations — a
 * three-way version deadlock), this copy is patched narrowly:
 *   1. `goog.soy.data.UnsanitizedText` is reconstructed locally instead of
 *      required from goog.soy.data (removed upstream).
 *   2. The three `goog.soy.*` convenience aliases are dropped; nothing in
 *      this project calls them (app code renders via the compiled
 *      template functions directly, see src/features/render.js).
 *   3. The removed `SanitizedContentKind.TEXT` sentinel is reconstructed
 *      as a local constant instead of read off the (now TEXT-less) shared
 *      enum object, so the ADVANCED type checker never has to accept an
 *      enum member the pinned closure-library doesn't declare.
 * Search this file for "PATCH:" to find every change.
 */
...
// PATCH: provided locally below instead of required from goog.soy.data
// (removed from google-closure-library after ~2018).
...
// PATCH: goog.require('goog.soy') removed — it pulled in goog.soy.renderElement
// et al., whose modern type signatures no longer match this runtime's
// deprecated soy.renderElement/renderAsFragment/renderAsElement aliases
// (removed below; unused by this project). No other symbol from the
// goog.soy module is referenced anywhere in this file.
goog.require('goog.soy.data.SanitizedContent');
goog.require('goog.soy.data.SanitizedContentKind');
goog.require('goog.soy.data.SanitizedCss');
// PATCH: goog.require('goog.soy.data.UnsanitizedText') removed — reconstructed
// locally below instead (removed from google-closure-library after ~2018).
goog.require('goog.string');
goog.require('goog.string.Const');
goog.require('goog.string.StringBuffer');

// PATCH: goog.soy.data.UnsanitizedText was removed from google-closure-library
// after ~2018 (this project pins ^20230802.0.0). soydata.UnsanitizedText below
// extends it purely for the prototype chain via goog.inherits — none of its
// own behavior is used — so this is a faithful, minimal reconstruction of the
// historical class (last seen in google-closure-library ~20180204).
/**
...
// PATCH: SanitizedContentKind.TEXT was removed from google-closure-library's
// goog.soy.data.SanitizedContentKind enum after ~2018 ("TEXT doesn't produce
// SanitizedContent anymore, use renderText" — this runtime predates that
// migration). Reconstructed as a local sentinel, not as a property on the
// shared enum object, so the ADVANCED type checker never has to accept an
// enum member the pinned closure-library doesn't declare.
/** @const {!goog.soy.data.SanitizedContentKind} */
var TEXT_CONTENT_KIND_COMPAT_ = /** @type {!goog.soy.data.SanitizedContentKind} */ (
    goog.DEBUG ? {sanitizedContentKindText: true} : {});
...
soydata.SanitizedHtml.from = function(value) {
  ...
  if (value instanceof goog.html.SafeHtml) {
    // PATCH: goog.html.SafeHtml.prototype.getDirection() was removed from
    // google-closure-library (bidi direction tracking dropped from
    // SafeHtml entirely). This project never passes direction-bearing
    // SafeHtml into a Soy template, so falling back to "unknown" (null)
    // instead of a known direction is a safe, unexercised degradation.
    return soydata.VERY_UNSAFE.ordainSanitizedHtml(
        goog.html.SafeHtml.unwrap(value), null);
  }
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(
...
soydata.UnsanitizedText.prototype.contentKind =
    // PATCH: was soydata.SanitizedContentKind.TEXT; see TEXT_CONTENT_KIND_COMPAT_.
    TEXT_CONTENT_KIND_COMPAT_;
...
// Public utilities.
//
// PATCH: soy.renderElement / soy.renderAsFragment / soy.renderAsElement
// removed. They were thin aliases over goog.soy.* (soy.js), whose modern
// type signatures no longer match this runtime's ARG_TYPES-based template
// function type, and no code in this project called them — app code
// renders compiled template output itself (see src/features/render.js).
// Removing them also drops the goog.require('goog.soy') dependency above,
// which is what actually pulled in the mismatched signatures.
...
soy.$$filterNoAutoescape = function(value) {
  // PATCH: was soydata.SanitizedContentKind.TEXT; see TEXT_CONTENT_KIND_COMPAT_.
  if (soydata.isContentKind_(value, TEXT_CONTENT_KIND_COMPAT_)) {
...
soy.$$bidiUnicodeWrap = function(bidiGlobalDir, text) {
  ...
  // PATCH: was soydata.SanitizedContentKind.TEXT; see TEXT_CONTENT_KIND_COMPAT_.
  if (soydata.isContentKind_(text, TEXT_CONTENT_KIND_COMPAT_)) {
